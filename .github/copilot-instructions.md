...
- Run from this repository:
  - `bash ./quality.sh`
  - `bash ./smoke.sh`
- If the change affects scripts used by global orchestration, run from `my-dashboard-backend`:
...
- Prefer deterministic, retry-aware checks for service readiness, migrations, and API smoke validation.
- Python utilities in this repository must be run with `python3`.
- New checks should include diagnostics that immediately expose failing service and failing command context.
